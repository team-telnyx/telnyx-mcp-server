...
async def create_assistant(request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Args:
        ...
        tools: Optional. List of tools for the assistant, each containing:
            - type: Required. Type of tool ("function" or "retrieval").
...
@mcp.tool()
async def update_assistant(assistant_id: str, request: Dict[str, Any]) -> Dict[str, Any]:
    """Update an AI Assistant. Once there is an agent created, you can talk the user about what can be updated in an easy manner, 
    rather than asking for a long list of fields to update.
...
async def start_assistant_call(
    ...
    try:
        ...
        if not assistant or not assistant.get("telephony_settings") or not assistant["telephony_settings"].get("default_texml_app_id"):
            raise ValueError("The assistant does not have a default TeXML application ID configured")
